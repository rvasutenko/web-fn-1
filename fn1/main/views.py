...
from .models import History, Material, News, Project
...
def about_page(request):
    ...
    for project in projects_qs:
        ...
        for pp in project.project_paragraphs.all():
            p = pp.paragraph
            ...
            if p.type in ("leftImg", "rightImg"):
                block = {
                    "img": p.image.url if p.image else "",
                    "title": p.title,
                    "text": p.text,
                    "reversed": p.type == "rightImg",
                }
                ...
                block = {
                    "button": {
                        "text": p.title,
                        "href": p.text,  # например, ссылка хранится в text
                    }
                }
...
def aboutFn_page(request):
    history_qs = (
        ...
        .filter(is_visible=True)
        .prefetch_related(
            "history_paragraphs__paragraph"
        )
    )

    history = []

    for story in history_qs:
        content = []

        for pp in story.history_paragraphs.all():
            p = pp.paragraph
    ...
    return render(request, "aboutFn.html", {
        'history': history,
        # 'history1': history1,
        'persolation': persolation,
        'persolation1': persolation1
    })
